...
const safeInlineImage = value => /^https:\/\/[^\s]+$/i.test(String(value || '')) || /^\/assets\/[^\s]+$/i.test(String(value || '')) ? String(value) : '';
function inlineMarkdown(value) {
  const links = [];
  const source = String(value || '').replace(/\[([^\]\n]+)\]\((https:\/\/[^\s)]+)\)/g, (_, label, url) => {
    const marker = `CMSLINK${links.length}TOKEN`;
  ...
  return escapeHtml(source)
    .replace(/\*\*([^*\n]+)\*\*/g, '<strong>$1</strong>')
    .replace(/\*([^*\n]+)\*/g, '<em>$1</em>')
    .replace(/CMSLINK(\d+)TOKEN/g, (_, index) => links[Number(index)] || '');
}
const paragraphs = value => String(value || '').split(/\n\s*\n/).map(item => item.trim()).filter(Boolean).map(block => {
  const image = block.match(/^!\[([^\]\n]{0,220})\]\((https:\/\/[^\s)]+|\/assets\/[^\s)]+)\)(?:\n\*([^*\n]{1,240})\*)?$/i);
  if (image) {
    const source = safeInlineImage(image[2]);
    if (source) return `<figure class="journal-inline-image"><img src="${escapeHtml(source)}" alt="${escapeHtml(image[1])}" loading="lazy" decoding="async">${image[3] ? `<figcaption>${escapeHtml(image[3])}</figcaption>` : ''}</figure>`;
  }
  ...
  if (lines.length && lines.every(line => /^[-*]\s+/.test(line))) return `<ul>${lines.map(line => `<li>${inlineMarkdown(line.replace(/^[-*]\s+/, ''))}</li>`).join('')}</ul>`;

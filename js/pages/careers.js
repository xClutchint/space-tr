...
const emptyReset = empty?.querySelector('[data-career-empty-reset]');
...
const careerCopy = {
  en: {
    ...
    inPerson: 'In person', hybrid: 'Hybrid', remote: 'Remote',
    ...
    initialStatus: 'No open positions at this time', role: 'Role', department: 'Department', type: 'Type', posted: 'Posted',
    ...
    noOpenings: 'No open positions at this time', newOpportunities: 'New opportunities will be published here.',
  ...
  fr: {
    ...
    inPerson: 'Sur site', hybrid: 'Hybride', remote: 'À distance',
    ...
    initialStatus: 'Aucun poste à pourvoir actuellement', role: 'Poste', department: 'Département', type: 'Contrat', posted: 'Publication',
    ...
    noOpenings: 'Aucun poste à pourvoir actuellement', newOpportunities: 'Les nouvelles opportunités seront publiées ici.',
...
function filterCareers(resetPage = false) {
  ...
  const hasNoPublishedRoles = rows.length === 0;
  if (hasNoPublishedRoles) {
    status.textContent = strings().initialStatus;
    if (emptyTitle) emptyTitle.textContent = strings().noOpenings;
    if (emptyBody) emptyBody.textContent = strings().newOpportunities;
    if (emptyReset) emptyReset.hidden = true;
  } else if (!matches.length) {
    status.textContent = strings().noFilterMatches;
    if (emptyTitle) emptyTitle.textContent = strings().noMatching;
    if (emptyBody) emptyBody.textContent = strings().tryAnother;
...
emptyReset?.addEventListener('click', resetCareers);
...
function workplaceLabel(value) {
  const copy = strings();
  return ({ ONSITE: copy.inPerson, HYBRID: copy.hybrid, REMOTE: copy.remote })[value] || copy.inPerson;
}
...
function renderCmsJobs(jobs) {
  list.replaceChildren(...jobs.map(job => {
    ...
    row.href = `/jobs/${encodeURIComponent(job.slug)}${preview ? `?preview=${encodeURIComponent(preview)}` : ''}`;
    ...
    const place = job.workplaceType === 'REMOTE' ? workplaceLabel('REMOTE') : `${workplaceLabel(job.workplaceType)} · ${job.location}`;
    const values = [job.title, departmentLabel(job.department), place, employmentLabel(job.employmentType)];
...
async function hydrateCareers() {
  try {
    const preview = new URLSearchParams(location.search).get('preview');
    const response = await fetch(preview ? `/api/content?preview=${encodeURIComponent(preview)}` : '/api/content', { headers: { Accept: 'application/json' } });
    ...
    // The checked-in empty state remains as a resilient fallback for local/static use.
    formatDates();
    filterCareers();
  }
}

(async()=>{
  ...
  let base=window.SPACE_MEDIA_CURATION||{version:1,hero:{excludeLightBackgrounds:true,excluded:[],included:[]},carousel:{excluded:[],included:[]}};
  try{
  ...
  // Mirrors the production hero pool in js/pages/home.js exactly.
  ...
  const signalCurationUpdate=curation=>{
    ...
    if('BroadcastChannel' in window){const channel=new BroadcastChannel('space-media-curation');channel.postMessage(message);channel.close()}
  };
  ...
  $('[data-save]').addEventListener('click',async()=>{
    ...
    try{
      ...
      const data=await response.json();state.curation=data.curation;signalCurationUpdate(state.curation);render();setSaveStatus('Approvals saved. Open homepage tabs updated.');
    }catch{

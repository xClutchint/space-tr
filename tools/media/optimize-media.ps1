param(
  [string]$AssetRoot = (Join-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) 'assets')
)

Add-Type -AssemblyName System.Drawing

$projectRoot = (Resolve-Path -LiteralPath (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent)).Path
$assetRootPath = (Resolve-Path -LiteralPath $AssetRoot).Path
$sourceRoot = Join-Path $assetRootPath 'media\source\brands'
$derivativeRoot = Join-Path $assetRootPath 'media\generated\derivatives'
...
$files = Get-ChildItem -LiteralPath $sourceRoot -Recurse -File | Where-Object {
  $_.Extension.ToLowerInvariant() -in @('.jpg','.jpeg','.png','.gif')
}

foreach($file in $files) {
  $relative = $file.FullName.Substring($projectRoot.Length+1).Replace('\','/')
  $sourceRelative = $file.FullName.Substring($sourceRoot.Length+1).Replace('\','/')
  $legacyRelative = "assets/$sourceRelative"
  $id = Get-StableId $legacyRelative
  try {
...
$analysisPath = Join-Path $assetRootPath 'media\source\media-analysis.json'

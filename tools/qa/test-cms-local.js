...
async function run() {
  ...
  const article={sourceId:'external-article-001',title:'A securely imported article',body:`This draft arrives through the restricted blog ingestion endpoint.\n\n## Market context\n\nSpace protects **brand value** through disciplined execution.\n\n![Retail training](${apiImage})\n*An adviser training session*`,excerpt:'A controlled draft import.',imageUrl:apiImage,language:'en'};
  ...
  const importedHtml=await (await fetch(`${origin}/en/posts/${importedSlug}`)).text(); assert.match(importedHtml,/<h2>Market context<\/h2>/); assert.match(importedHtml,/<strong>brand value<\/strong>/); assert.match(importedHtml,/<figure class="journal-inline-image">/); assert.match(importedHtml,/<figcaption>An adviser training session<\/figcaption>/);

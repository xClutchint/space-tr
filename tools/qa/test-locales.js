...
const { pages } = require('../build/generate-localized-pages');
...
const frenchBodyMarkers = {
  ...
  'posts.html': '>Parcourir<',
  'careers.html': 'Postes à pourvoir'
};

for (const language of ['en', 'fr']) {
  for (const page of pages) {
    const filename = path.join(pagesRoot, language, page.slug || 'index.html');
    ...
    if (!html.includes('/js/shared/locale-routing.js')) failures.push(`${filename}: missing deterministic locale bootstrap`);

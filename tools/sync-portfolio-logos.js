 ...
 * Run with `node tools/sync-portfolio-logos.js` after making Sharp available.
 * The script writes only the missing niche logos, brand_41 through brand_54,
...
const destination = path.join(root, 'brand kit', 'brands');
...
async function writeVariants(number, payload, kind) {
  ...
  await fs.writeFile(path.join(destination, `brand_${number}_mark.png`), mark);
  await sharp({ create: { width: 144, height: 144, channels: 3, background: '#ffffff' } })
    ...
    .toFile(path.join(destination, `brand_${number}.avif`));
}
